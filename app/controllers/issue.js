    ...
    //使用findOne,只取最后插入的一条
        ...
        .exec(function (err, issue) {
          res.render('issueDetail', {
            title: issue.title, lines: lines, issue: issue
          })
        })
  })
};
...
exports.update = function (req, res) {
  ...
  if (id) {
    Issue.findById(id, function (err, issue) {
      Line.find({members: userId}, function (err, lines) {
        res.render('issue', {
          title: '需求更新页面', issue: issue, lines: lines
        })
      })
    })
  }
      ...
      _issue.save(function (err, issue) {
        ...
        function arr_diff(a1, a2) {
          ...
          for (var i = 0; i < a2.length; i++) {
            if (ROLE[a2[i]]) delete ROLE[a2[i]]; else ROLE[a2[i]] = true;
          }
          for (var r in ROLE) {
...
exports.myAllocatedJson = function (req, res) {
  ...
  if (userId) {
    ...
    Line.find({members: userId}, function (err, lines) {
      ...
      //只取title,start,end字段
          .find({belongLineId: {$in: lineIdArray}, role: {"$in": [userRole]}, members: {$in: [userId]}},
              {_id: 0, title: 1, start: 1, end: 1, url: 1})
          .populate('belongLineId', 'name')
          .exec(function (err, issues) {
            if (err) {
              console.log(err)
  ...
  if (id) {
    Issue.findById(id, function (err, issue) {
      if (err) {
        ...
        Issue.update({_id: id, 'allocate.roleType': role},
            {$set: {'allocate.$.allocated': true, 'allocate.$.memberId': memberId}}).exec();
        res.json({success: 1});
      }
    })
  }
          ...
          .exec(function (err, issues) {
            ...
            if (err) {
              ...
              res.render('lineIssueList', {
                q: q, title: '业务线-需求列表', currentPage: (page + 1), //向上舍入取整
                totalPage: Math.ceil(issues.length / count), issues: results
              })
            }
          })
    })
